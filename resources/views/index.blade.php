...
@section('content')
    ...
    <div class="col-xl-3 col-md-6">
      <div class="card bg-warning text-white mb-4">
        <div class="card-body d-flex flex-row justify-content-between">
          ...
          <div></div>
          <div class="mb-5 mr-5">
            <i class="fas fa-solid fa-game m-2" style="font-size: 7rem; position:absolute; opacity:0.3"></i>
          </div>
        </div>
        <div class="card-footer d-flex align-items-center justify-content-between">
          <a href="{{ route('games.index') }}" class="small text-white stretched-link">Подробнее</a>
          <div class="text-white ml-auto p-2">
            <i class="fas fa-angle-right"></i>
          </div>
        </div>
  ...
  <div class="row d-flex flex-row justify-content-around">
    ...
    <div class="col-xl-5 m-2">
      <div class="card mb-4">
        <div class="card-header">
          <p class="chart_cart">Топ 5 игр по оценкам</p>
        </div>
        <div class="card-body" style="height: 500px; max-height: 500px;">
  ...
  <script>
    ...
    let categoryGameCounts = @json($categoryGameCounts);

    let labels = categoryNames;
    let data = categoryGameCounts.map(item => item.total);
  ...
  <script>
    let gameNames = @json($gameNames);

    let topRatedGames = @json($topRatedGames);
    let topRatedGamesArray = Object.values(topRatedGames);

    let labelstwo = gameNames;
    let datatwo = topRatedGamesArray.map(item => item.average_rating);
