...
@section('content')
  <div class="row">
    <div class="col-12">
      <div class="mb-3">
        <a href="{{ route('games.create') }}" class="btn btn-success button-savet">Добавить</a>
      </div>
      <div class="card">
        <div class="card-header border-0 m-2">
          <h3 class="card-title">Таблица Игр</h3>
    ...
    <div class="container" style="width: 70%; max-width: 70%;">
      <div class="row mt-3">
        @foreach ($games as $game)
          <div class="col-sm game mt-2">
            <div class="card card-game" style="width: 17rem; min-height:670px; max-height: 670px; max-width:100%;">
              <div class="card-head">
                <div class="overlay" style="background-image: url('{{ $game->link_img }}');"></div>
                <a href="{{ route('gameinfo.show', $game->id) }}">
                  <div class="card-image-container">
                    <img src="{{ $game->link_img }}" alt="Game" class="card-img-top rounded mr-3 card-image"
              ...
              <div class="card-body">
                <h3 class="dropdown-item-title">
                  <div class="d-flex flex-col w-auto">
                    <a href="{{ route('gameinfo.show', $game->id) }}">{{ $game->name }}</a>
                    <p class="ml-2">{{ $game->age }}+</p>
                  </div>
                  <span class="float-right text-sm text-danger"><i class="fas fa-star"></i></span>
                </h3>
                <p class="text-sm">Дата выхода: {{ $game->year_of_issue }}</p>
                <p class="text-sm">Продолжительность: {{ $game->duration }} мин.</p>
              </div>
              <div class="card-footer d-flex flex-row align-items-end mt-auto justify-content-end">
                <a href="{{ route('games.edit', $game->id) }}" class="btn btn-success button-save mx-auto">Изменить</a>
  ...
  <script>
    window.onload = function() {
      ...
      search.addEventListener('keyup', function() {
        ...
        let movies = document.querySelectorAll('.game');
